$(function() {
  ...
  var $toDoList = $('#to-do-div');

  //right now calling our template from html
  var todoTemplate = _.template($('#to_do_list-template').html());
...
_.each(toDos, function (item, index) {
  var $varOfArr = $(todoTemplate(item));
  $varOfArr.attr('data-index', index);
  $toDoList.append($varOfArr);
});
  ...
  $newToDo.on('submit', function(event) {
    ...
    $toDoList.append('<ul class="todo">' + toDoData.name + ' - ' + toDoData.desc + '</ul>');
